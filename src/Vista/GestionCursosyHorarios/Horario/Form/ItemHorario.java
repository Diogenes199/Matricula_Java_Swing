...
import Vista.GestionCursosyHorarios.Horario.ModelsAdapter.HorarioTableModel;
import javax.swing.ImageIcon;
import javax.swing.JOptionPane;

public class ItemHorario extends javax.swing.JInternalFrame {
    private final HorarioControlador controlador = new HorarioControlador();
    private final HorarioTableModel tableModel;

    private int valor;

    public ItemHorario() {
        initComponents();
        tableModel = new HorarioTableModel(controlador);
        tblHorario.setModel(tableModel);
        ObtenerTodos();
        panelHorario.setEditable(false);
        panelHorario.setHorario(null);
        rbTodos.setSelected(true);
        txt_Buscado.requestFocus();
        valor = 0;

        this.tblHorario.getSelectionModel().addListSelectionListener(e
                -> {
            activarBotonesCRUD(tblHorario.getSelectedRow() != -1 && tblHorario.getColumnCount() > 1);
            txt_Buscado.setText(tblHorario.getColumnCount() > 1 ? "" : txt_Buscado.getText());
        });
    ...
    private void ObtenerTodos() {
        tableModel.getAll();
        tableModel.centrarContenido(tblHorario);
        tableModel.ajustarTamanioColumnas(tblHorario);
        tableModel.fireTableDataChanged();
    }

    private Horario getSelection() {
        String id = tblHorario.getValueAt(tblHorario.getSelectedRow(), 0).toString();
    ...
    private void initComponents() {
        ...
        panelHorario = new Vista.GestionCursosyHorarios.Horario.Form.PanelHorario();
        ...
        rbHorario.setFont(new java.awt.Font("Times New Roman", 1, 13)); // NOI18N
        rbHorario.setText("Dia");
    ...
    private void btn_NuevoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_NuevoActionPerformed
        ...
        panelHorario.setEditable(true);
        panelHorario.setHorario(null);
        panelHorario.dataLoad();
        panelHorario.setCod(controlador.newCode());
        activarBotonesCRUD(false);
    ...
    private void btn_BorrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_BorrarActionPerformed
        if (JOptionPane.showConfirmDialog(null, "¿Seguro que quiere borrar el Curso (" + tblHorario.getValueAt(tblHorario.getSelectedRow(), 1).toString() + ")?", "BORRAR CURSO ", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {
            String id = tblHorario.getValueAt(tblHorario.getSelectedRow(), 0).toString();
    ...
    private void btn_EditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_EditarActionPerformed
        ...
        activarBotonesGuardar(true);
        panelHorario.setEditable(true);
        panelHorario.setHorario(getSelection());
        panelHorario.dataLoad();
    ...
    private void btn_GuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_GuardarActionPerformed
        ...
        panelHorario.dataSave();
        Horario existe = controlador.getOne(panelHorario.getCod());
        Horario insertar = panelHorario.getHorario();
        if (existe != null) {
            estado = controlador.update(insertar);
            message = estado ? "Usuario Actualizado con exito" : "Espera (*)";

        }
        if (existe == null) {
            estado = controlador.create(insertar);
    ...
    private void btn_BuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_BuscarActionPerformed
        if (txt_Buscado.isEnabled()) {
            ...
            HorarioTableModel tablita = new HorarioTableModel(controlador);
            switch (valor) {
                case 0 -> tablita.getAll();
                case 1 -> tablita.getOne(textoBuscado);
                case 2 -> tablita.getHorarios(textoBuscado);
                default -> {
                }
            }

            tblHorario.setModel(tablita);
            tablita.centrarContenido(tblHorario);
            tablita.ajustarTamanioColumnas(tblHorario);
        }
    ...
    private Vista.GestionCursosyHorarios.Horario.Form.PanelHorario panelHorario;
